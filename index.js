...
function sorting (logs) {
  return logs.sort((item1, item2) => {
    const { log1, log2 } = getTime(item1, item2);
    if(log1 === log2){
      return 0;
    }
...
function getTime(item1, item2) {
  return {
    log1: new Date(item1.date).getTime(),
    log2: new Date(item2.date).getTime()
  }
}
